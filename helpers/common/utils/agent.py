...
def _invoke_transfer_to_agent(**kwargs) -> bool:
...
def transfer_to_socio_agent(**kwargs) -> bool:
    ...
    if _invoke_transfer_to_agent(request=request, cx_agent_platform=CxAgentPlatforms.SOCIO, cx_channel=CxChannels.FACEBOOK, intent_id=intent_id, meta_data=meta_data, spiel=spiel):
        return True
    return False
...
def transfer_to_live_person_agent(**kwargs) -> bool:
    ...
    if _invoke_transfer_to_agent(request=request, cx_agent_platform=CxAgentPlatforms.LIVE_PERSON, cx_channel=CxChannels.FACEBOOK, support_intent=support_intent, meta_data=meta_data, spiel=spiel):
        return True
    return False


def transfer_to_connect_agent(**kwargs) -> bool:
    return True


def generic_transfer_to_agent(**kwargs):
    request = kwargs.get("request", {})
    session_tools = kwargs.get("session_tools", {})
    last_brand = session_tools.get_last_brand()
    page_id = session_tools.get_page_id()
    meta_data = kwargs.get("meta_data", {})

    agent_channel = "liveperson"
    intent_id = configs.socioBasket.get(page_id, {}).get("main", "")

    SUPPORT_INTENT_LOB_MAP = {
        str_const.LOB_NAME_PREPAID              : "prepaid-regular",
        str_const.LOB_NAME_TM                   : "prepaid-regular",
        str_const.LOB_NAME_POSTPAID             : "postpaid-regular",
        str_const.LOB_NAME_PREPAID_HOME_WIFI    : "hpw-regular",
        str_const.LOB_NAME_BROADBAND            : "broadband-regular"
    }

    if last_brand in SUPPORT_INTENT_LOB_MAP:
        agent_channel = "liveperson"
        support_intent = SUPPORT_INTENT_LOB_MAP.get(last_brand, "")

    if agent_channel == "liveperson":
        transfer_to_live_person_agent(request=request, support_intent=support_intent, meta_data=meta_data)
    else:
        transfer_to_socio_agent(request=request, intent_id=intent_id, meta_data=meta_data)