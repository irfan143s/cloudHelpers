...
from resources.spiels import (
    ...
    FB_PUBLIC_NOTIFY_USER_ACTIVE_PAGE_SPIEL,
    FB_PUBLIC_NOTIFY_USER_CURRENT_PAGE_SPIEL,
    FB_PUBLIC_OPEN_TICKET_SPIEL,
    YES_NO_MENU,
    MENU_FORM
)
...
class FbPagesSingleIdentity:
    ...
    def __init__(self, page_id: str, psid: str, identity_key: str = ""):
        ...
        self.__ddb_fbui = DDBTools(DDB_FACEBOOK_USER_IDENTITY, REGION_OREGON)
        self.__facebook_tools = FacebookTools()
        self.__response_formatter = ResFormatter(self.__page_id)
        self.__ddbutils = DDBUtils()
        ...
        self.__is_identity_key_exists = True if self.__identity_key.strip() else False
    ...
    def __init_identity_check(self) -> None:
        ...
        is_user_and_main_identity_keys_equal = True
        ...
        if not self.__is_identity_key_exists:
            ...
            # if current psid does not exists on identityTable, call ids_for_pages API
            if self.__user_identity_data:
                logger.info(f"Current PSID exists on user identity data.")
        ...
        if not self.__user_identity_data:
            ...
            if not self.__user_identity_data:
                ...
                is_user_and_main_identity_keys_equal = False

        logger.info(f"User identity data: {self.__user_identity_data}")
        self.__identity_key = self.__user_identity_data["identityKey"]

        # If identity key exists in MAIN and current psid does not exists on user identity data, add it.
        if not PSID_KEY_ATTRIBUTES[self.__page_id]["attribute_name"] in self.__user_identity_data.keys():
            logger.info(f"Current PSID does not exists on user identity data, adding it...")
            self.__ddb_fbui.update_item("identityKey", self.__identity_key, {PSID_KEY_ATTRIBUTES[self.__page_id]["attribute_name"]: self.__psid})
        ...
        if not is_user_and_main_identity_keys_equal:
    ...
    def __get_user_psids_from_identity_data(self) -> dict:
        ...
        for pageid, value in PSID_KEY_ATTRIBUTES.items():
            if not value["attribute_name"] in self.__user_identity_data.keys():
                continue
    ...
    def __get_main_psids(self) -> list:
        ...
        responses = self.__ddbutils.batch_get_item(REGION_OREGON, batch_keys)
    ...
    def __get_fb_name(self) -> tuple:
        try:
            ...
            fname = result["first_name"] if result and "first_name" in result else " "
            lname = result["last_name"] if result and "last_name" in result else " "
        except Exception as error:
    ...
    ****************************************************************************************************************** """
    ...
    def has_active_bot_session(self) -> bool:
        logger.info(f"Checking if user has an active BOT session.")
        if not self.__user_psids:
            return False
        ...
        responses = self.__ddbutils.batch_get_item(REGION_OREGON, batch_keys)
        logger.info(f"Session tables Responses: {responses}")
        ...
        logger.info(f"User has no active BOT session.")
        self.__has_an_active_bot_session = False
        return False
    ...
    def has_active_session(self, is_fresh: bool = True) -> bool:
        ...
        if is_fresh:
            if self.has_active_agent_session() or self.has_active_bot_session():
                return True
            return False
    ...
    def notify_user_on_active_page_from_private_reply(self, notif_spiel: str = "") -> None:
        active_page_id = self.get_active_page_id()
        active_psid = self.get_active_psid()
        resformatter = ResFormatter(active_page_id)

        logger.info(f"Notifying user from the active page (from private reply). page_id={active_page_id}, psid={active_psid}")   

        if self.__active_entity == ENTITY_BOT:
            ddb_main_active = DDBTools(page_ddb_mapping_dict[active_page_id]['main'], REGION_OREGON)
            ddb_main_active.update_item("fbId", active_psid, {"sessionState": MAIN_MENU_STATE, "subState" : "1"})
            
            if not notif_spiel:
                notif_spiel = FB_PUBLIC_NOTIFY_USER_ACTIVE_PAGE_SPIEL.format(page_name_mapping_dict[self.__page_id])  
                
            resformatter.send_message(active_psid, notif_spiel, True)
            resformatter.send_quickresponse(active_psid, FB_PUBLIC_OPEN_TICKET_SPIEL ,YES_NO_MENU, MENU_FORM, True)
        else:
            logger.info(f"User is active on AGENT on {page_name_mapping_dict[active_page_id]}, we'll not notify.")
      
            
    def notify_user_on_current_page_via_private_reply(self, notif_spiel: str = "", comment_id: str = "") -> None:
        logger.info(f"Notifying user from current page via private reply. page_id={self.__page_id}, psid={self.__psid}")
        active_page_id = self.get_active_page_id()
        if not self.__response_formatter:
            self.__response_formatter = ResFormatter(self.__page_id)
        else:   
            if not notif_spiel:
                notif_spiel = FB_PUBLIC_NOTIFY_USER_CURRENT_PAGE_SPIEL.format(page_name_mapping_dict[self.__page_id],page_name_mapping_dict[active_page_id])

        options_spiel = self.__get_current_page_options_spiel(active_page_id, notif_spiel)
        buttons = options_spiel["buttons"]
        spiel = options_spiel["spiel"]
        self.__response_formatter.send_option_buttons(comment_id, spiel, buttons, True, True, True)
